...
export function TopNavbar({ title }: { title: string }) {
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-4 border-b border-border bg-background/95 px-8 backdrop-blur-md transition-colors duration-200">
      <div className="flex min-w-0 items-center gap-3">
        <span className="shrink-0 text-sm font-semibold tracking-tight text-foreground">HireLens</span>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="h-9 w-9 rounded-full bg-muted ring-1 ring-border transition-all duration-200 hover:ring-border-hover" title="Account" />
      </div>
    </header>
  );
}
